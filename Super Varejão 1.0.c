...
struct Produto
    {
        ...
        int id,qtdEstoque,vendas,ativo;
    };
...
return 0;
    ...
    void ReporEstoque(struct Produto *produtos)
    {
        ...
        if(verificador != 0)
        {
            ...
            {
                if(id == produtos[i].id && produtos[i].ativo == 1) //Mostra as informações do ID digitado.
                {
    ...
    {
        double divide=0;
        int ativacao; //Le o input para desativar o produto
    ...
    void Inventario(struct Produto *produtos)
    {
        ...
        printf("\nID |          DESCRIÇÃO             |    ESTOQUE    | PRECO CUSTO  | PRECO VENDA  |  MARGEM LUCRO  |  ATIVO");
        printf("\n------------------------------------------------------------------------------------------------------------");
            for(int i=1;i<contProdutos;i++) //Loop para printar as informações do array produtos[]
            {
                printf("\n%2.1d |%-30.30s  |%8.1d UNDs  |R$%9.2lf   |R$%9.2lf   |%9.2lf %%     | %4.1d",produtos[i].id,produtos[i].nome,produtos[i].qtdEstoque,produtos[i].precoCusto,produtos[i].precoVenda,produtos[i].margemLucro,produtos[i].ativo);
                printf("\n------------------------------------------------------------------------------------------------------------");
            }
